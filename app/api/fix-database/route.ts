  ...
  try {
    ...
    if (checkError || !functionExists || functionExists.length === 0) {
      ...
      const { error: createError } = await supabase
        ...
        .catch(() => {
          // 如果 execute_sql 不存在，直接執行 SQL
          return supabase.query(createFunctionQuery)
        })

      if (createError) {
        console.error("創建 execute_sql 函數時出錯:", createError)
        // 繼續執行，因為可能是函數已存在
      }
    }
    ...
    // 執行修復腳本
    const { error } = await supabase.rpc("execute_sql", { sql_query: fixScript })
    ...
    if (error) {
      ...
      // 嘗試直接執行 SQL
      try {
        await supabase.query(fixScript)
      } catch (directError: any) {
        console.error("直接執行 SQL 時出錯:", directError)
        return NextResponse.json({ success: false, error: error.message }, { status: 500 })
      }
    }

    return NextResponse.json({ success: true, message: "數據庫修復完成" })
  } catch (error: any) {
    console.error("修復數據庫時出錯:", error)
    return NextResponse.json({ success: false, error: error.message }, { status: 500 })
  }
}
