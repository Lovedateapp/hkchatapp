...
export async function GET(request: Request) {
  try {
    ...
    if (createTableError) {
      return NextResponse.json({ error: `創建表失敗: ${createTableError.message}` }, { status: 500 })
    }
    ...
    if (rlsError) {
      return NextResponse.json({ error: `設置RLS失敗: ${rlsError.message}` }, { status: 500 })
    }
    ...
    if (insertError) {
      return NextResponse.json({ error: `插入管理員記錄失敗: ${insertError.message}` }, { status: 500 })
    }
