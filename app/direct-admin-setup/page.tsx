...
export default function DirectAdminSetupPage() {
  ...
  const handleSetupAdmin = async () => {
    ...
    try {
      ...
      if (createTableError) {
        setDebugInfo(`創建表失敗: ${createTableError.message}`)
    ...
    <div className="container mx-auto py-8 px-4 flex justify-center items-center min-h-[80vh]">
      <Card className="w-full max-w-md">
        <CardHeader>
          ...
          <CardDescription>使用SQL直接設置管理員</CardDescription>
        </CardHeader>
        <CardContent>
          {success ? (
            ...
            <div className="space-y-4">
              <p className="text-sm">此操作將使用SQL直接設置管理員，繞過RLS限制。</p>
          ...
          {debugInfo && (
            <div className="mt-4 p-2 bg-gray-50 border border-gray-200 rounded text-xs overflow-auto max-h-32 whitespace-pre-wrap">
              <pre>{debugInfo}</pre>
            </div>
          )}
        </CardContent>
